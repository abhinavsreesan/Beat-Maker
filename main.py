...
pygame.display.set_caption('Sick Beats Maker')
...
# function to play sounds when clicked
def play_notes():
...
# Function to draw and update the pygame grid
def draw_grid(clicks, beat, actives):
    ...
    # code to create the instrument names
    ...
    # code to create/update grid to select instrument selection
...
while run:
    ...
    bpm_add_rect = pygame.draw.rect(screen, gray, [510, HEIGHT-150, 48, 48], 0, 0)
    ...
    beats_add_rect = pygame.draw.rect(screen, gray, [810, HEIGHT - 150, 48, 48], 0, 0)
    ...
    # code to play the beat when clicked
    if beat_changed:
        play_notes()
        beat_changed = False

    # code to set and rest the select box in the grid
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONUP and not save_menu and not load_menu:
            # check if a box has been clicked
            if play_pause.collidepoint(event.pos):
                if playing:
                    playing = False
                elif not playing:
                    playing = True
            # check if add/sub bpm button clicked
            elif bpm_add_rect.collidepoint(event.pos):
                bpm += 5
            elif bpm_sub_rect.collidepoint(event.pos):
                bpm -= 5
            # check if add/sub beats button clicked
            ...
            # check if clear button clicked
                ...
                clicked = [[-1 for _ in range(beats)] for _ in range(instruments)]
            # check if save/load beats button clicked
            elif save_button.collidepoint(event.pos):
                save_menu = True
            elif load_button.collidepoint(event.pos):
                load_menu = True

            # code to check if an instrument is selected
